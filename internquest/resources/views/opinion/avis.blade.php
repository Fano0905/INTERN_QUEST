...
<div class="flex justify-center items-center min-h-screen">
    ...
    <form action="{{route('companies.e_store', $company->id)}}" method="POST">
        ...
        <div class="relative mb-2 flex flex-col">
            ...
            <input type="number" name="company_id" id="company_id" value="{{$company->id}}" data-name="{{ $company->name }}" title="{{ $company->name }}" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400">
            @error('company_id')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
        <div class="relative mb-2 flex flex-col">
            ...
            <input type="text" name="title" id="title" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400" required>
            @error('title')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
        <div class="relative mb-2 flex flex-col">
            ...
            <textarea name="comment" id="comment" cols="50" rows="10" placeholder="Leave a comment" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400"></textarea>
            @error('comment')
            <p style="color: red;">{{$message}}</p>
        @enderror
        </div>
