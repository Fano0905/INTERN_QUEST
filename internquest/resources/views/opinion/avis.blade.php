...
<div class="flex justify-center items-center min-h-screen">
    ...
    <form action="{{route('companies.e_store', $company->id)}}" method="POST">
        ...
        <div class="relative mb-2 flex flex-col">
            ...
            <input type="text" name="title" id="title" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400" required>
            @error('title')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
        <div class="relative mb-2 flex flex-col">
            ...
            <textarea name="comment" id="comment" cols="50" rows="10" placeholder="Leave a comment" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400"></textarea>
            @error('comment')
            <p style="color: red;">{{$message}}</p>
        @enderror
        </div>
