...
<div class="evaluation">
    <form action="{{route('companies.e_update', $evaluation->id)}}" method="POST">
        ...
        <div class="relative mb-6">
            ...
            <input type="number" name="company_id" id="company_id" value="{{$evaluation->company_id}}" data-name="{{ $company->name }}" title="{{ $company->name }}" class="w-full pl-10 pr-3 py-1 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400">
            @error('company_id')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
        <div class="relative mb-6">
            ...
            <input type="text" name="title" id="title" value="{{$evaluation->title}}" class="w-full pl-10 pr-3 py-1 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400" required>
            @error('title')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
        <div class="relative mb-6">
            ...
            <textarea name="comment" id="comment" cols="50" rows="10" placeholder="Laisser un commentaire" class="w-full pl-10 pr-3 py-1 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400">{{$evaluation->comment}}</textarea>
            @error('comment')
            <p style="color: red;">{{$message}}</p>
        @enderror
        </div>
