    ...
    <div class="w-full max-w-xl bg-white p-10 rounded-lg shadow-lg">
        <form action="{{route('companies.e_update', $evaluation->id)}}" method="POST">
            ...
            <div class="relative mb-2 flex flex-col">
                ...
                <input type="number" name="user_id" id="user_id" value="{{Auth::user()->id}}" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400">
                @error('user_id')
            ...
            <div class="relative mb-2 flex flex-col">
                ...
                <select name="note" id="note" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400">
            ...
            <div class="relative mb-2 flex flex-col">
                ...
                <input type="number" name="company_id" id="company_id" value="{{$evaluation->company_id}}" data-name="{{ $company->name }}" title="{{ $company->name }}" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400">
                @error('company_id')
                    <p style="color: red;">{{$message}}</p>
                @enderror
            </div>
            <div class="relative mb-2 flex flex-col">
                ...
                <input type="text" name="title" id="title" value="{{$evaluation->title}}" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400" required>
                @error('title')
                    <p style="color: red;">{{$message}}</p>
                @enderror
            </div>
            <div class="relative mb-2 flex flex-col">
                ...
                <textarea name="comment" id="comment" cols="50" rows="10" placeholder="Laisser un commentaire" class="w-full pl-3 pr-3 py-2 bg-transparent border-b-2 border-red-600 outline-none focus:border-red-400">{{$evaluation->comment}}</textarea>
                @error('comment')
                <p style="color: red;">{{$message}}</p>
            @enderror
            </div>
