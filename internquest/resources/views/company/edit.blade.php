...
<body>
    ...
    <form action="{{route('companies.update', $company->id)}}" method="POST" enctype>
        ...
        <div class="relative mb-6">
            <label required class="absolute left-2 -top-4 text-base text-gray-700 font-medium transition-all">Secteur</label>
        ...
        <div class="relative mb-6">
            ...
            <input type="text" name="website" id="website" required placeholder="example.fr" value="{{$company->website}}" class="w-full pl-10 pr-3 py-1 bg-transparent border-b-2 border-blue-600 outline-none focus:border-blue-400">
            @error('website')
                <p style="color: red;">{{$message}}</p>
            @enderror
        </div>
