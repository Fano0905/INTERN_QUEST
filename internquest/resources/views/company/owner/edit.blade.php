...
<div id="insert_dialog" class="fixed inset-0 m-auto w-100 h-110  bg-transparent border-2 border-white border-opacity-50 rounded-3xl shadow-2xl flex items-center justify-center overflow-hidden">
    <div class="w-full p-10 flex flex-col items-center">
        <h2 class="text-2xl text-blue-600 mb-6">Modifier le propriétaire</h2>
        <form action="{{route('owners.update', $company->id)}}" method="POST" class=w-full>
            @csrf
        ...
        <div>
            <button type="submit" class="w-full h-11 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-400 font-medium">Modifier l'assignation</button>
        </div>
    </form>
    <a href="{{ url()->previous() }}">
